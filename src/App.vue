    ...
    <div class="container">
      <div class="login" v-if="state.username===''||state.username===null">
        <form @submit.prevent='login'>
          ...
          <input type="text" placeholder="please enter name .. " v-model="inputLogin">
      ...
      <div class="chat" v-else>
        ...
        <main>
          <div class="message-box" :class="state.username===message.username&&'current-user'" v-for="message in state.messages" :key="message.id">
            <div class="user-icon">
              <!-- A -->
              {{message.username[0]}}
            </div>
          ...
          <!-- <div class="message-box">
            ...
            <div class="message">
              Voluptate exercitationem laboriosam blanditiis illo
            </div>
          </div> -->
        </main>
        <footer>
...
import db from "./db";
import {ref as rr, push,onValue} from "firebase/database";
export default {
  // name: 'App'
  // ,components: {
  //   HelloWorld,
  ...
  setup()
  {
    ...
    const sendMessage=()=>{
      ...
      // alert(Object.entries(db))
      const message={
        username:state.username,
        body:inputMessage.value
      }
      // messagesRef.push(message);
      push(rr(db, 'message/'), message);
      inputMessage.value="";
    }
    onMounted(()=>{
      const starCountRef = rr(db,'message/');
      ...
      // alert(messages[0].id)
      // alert(data['-N-hefZVjGC7HyS7vzWy'].username)
      // alert(data['-N-hefZVjGC7HyS7vzWy'].body   )
      // updateStarCount(messages, data);
      });
    })
    return{inputLogin,login,logout,state,sendMessage,inputMessage}
  }
}
</script>
